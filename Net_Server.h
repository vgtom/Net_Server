...
public:
	...
	void AddToBuffer(char* szBuffer)
	{
		m_CompleteData += szBuffer;
	}

	int GetBufferLen(void)
	{
		return m_CompleteData.size();
	}

	void GetBuffer(char *szBuffer)
	{
		//strcpy(szBuffer, m_szBuffer);
		strcpy(szBuffer, m_CompleteData.c_str());
	}
...
public:
	...
	static HANDLE g_hIOCompletionPort;
};

HANDLE ServerContext::g_hIOCompletionPort = NULL;
